...
int getLineErrorPosition() //Decide how far off of the line the robot is
{
...
task main()
{
  ...
  while (true)
  {
    ...
    nErrorValue = getLineErrorPosition(); //retrieve the error from the function
    nDerivative = nErrorValue - nLastError; // find the change in error from last time
    nAdjustment = ((nErrorValue * nPFactor) + (nDerivative * nDFactor))/ 100; // Decide on the best adjustment
    nLastError  = nErrorValue; //Store the current error to use in next cycle's Derivative value

    if (nAdjustment > 0) //sets the motors to adjust to the error in order to maintain a steady course.
    {
      nLeftMotor  = nMaxSpeed +  8 * nAdjustment / 2; //The adjustment is modified to produce a swing turn in the correct direction
      nRightMotor = nMaxSpeed - 10 * nAdjustment / 2;
    }
    else
    {
      ...
      nRightMotor = nMaxSpeed -  4 * nAdjustment / 2;
		...
		//Stop the motors if they have been disabled
    if (bMotorsDisabled)
    {
      motor[leftMotor]  = 0;
      motor[rightMotor] = 0;
    ...
    //This measures the SPI message counts and waits until there is a difference in count before breaking out of the while loop and continuing
    while (true)
    {
